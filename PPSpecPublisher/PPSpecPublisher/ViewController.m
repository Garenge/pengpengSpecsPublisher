...
static NSString * const kSpecRepoFolderPathKey = @"kSpecRepoFolderPathKey";

@synthesize specFolderPath = _specFolderPath;
- (NSString *)specFolderPath {
    if (nil == _specFolderPath) {
        // 从 UserDefaults 获取保存的路径
        NSString *savedPath = [[NSUserDefaults standardUserDefaults] stringForKey:kSpecRepoFolderPathKey];
        if (savedPath.length > 0 && [NSFileManager.defaultManager fileExistsAtPath:savedPath]) {
            _specFolderPath = savedPath;
        }
        // 默认硬编码路径（注释保留，以防以后找不到用的）
...
- (void)setSpecFolderPath:(NSString *)specFolderPath {
    ...
    if (specFolderPath.length > 0) {
        BOOL isDirectory = NO;
        BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:specFolderPath isDirectory:&isDirectory];
        isValidPath = exists && isDirectory;
    }

    if (!isValidPath) {
        [self showInvalidPathAlert];
        return;
    }
    ...
    [self refreshDataAfterDelay:0.5];

    // 保存到 UserDefaults
    [[NSUserDefaults standardUserDefaults] setObject:specFolderPath forKey:kSpecRepoFolderPathKey];
...
- (void)refreshDataAfterDelay:(NSTimeInterval)delay {
    if (delay > 0) {
...
/// 显示路径无效提示弹窗
- (void)showInvalidPathAlert {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"路径无效" preferredStyle:UIAlertControllerStyleAlert];
